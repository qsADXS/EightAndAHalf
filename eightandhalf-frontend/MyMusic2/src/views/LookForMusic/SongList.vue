<template>
  <div class="music-chart">
    ...
    <div class="chart-list">
      <h1>{{ $route.query.playlistName }}</h1>
      <img :src="$route.query.playListCoverUrl" alt="cover" class="list-cover" />
      <br />
      <br>
      <br>
...
import { collectMusicList } from '@/api/music/collectmusiclist'; // 导入收藏歌单的接口
...
export default {
  ...
  methods: {
    ...
    async getRankList() {
      try {
        const res = await getContentofList(this.playlistid);
        console.log(res);
    ...
    async collect() {
      try {
        const res = await collectMusicList(this.playlistid, 1); // type = 1 表示收藏
        if (res.base.code === 10000) {
          this.$message.success('收藏成功！');
        } else {
          this.$message.error(`收藏失败：${res.base.message}`);
        }
      } catch (error) {
        console.error('收藏歌单时发生错误:', error);
  ...
  created() {
    ...
    this.coverimage = this.$route.query.playListCoverUrl;
  },
};
</script>
